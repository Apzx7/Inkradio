...
contextBridge.exposeInMainWorld('desktopWindow', {
  ...
  // ─── LX 音源操作 ──────────────────────────────────────────────
  importLxSource: () => ipcRenderer.invoke('lx-source:import'),
  importLxSourceCode: (code) => ipcRenderer.invoke('lx-source:import-code', code),
  deleteLxSource: (id) => ipcRenderer.invoke('lx-source:delete', id),
  listLxSources: () => ipcRenderer.invoke('lx-source:list'),
  searchLxSource: (sourceId, keyword, page) => ipcRenderer.invoke('lx-source:search', sourceId, keyword, page || 1),
  getLxSourceUrl: (sourceId, musicInfo, quality) => ipcRenderer.invoke('lx-source:get-url', sourceId, musicInfo, quality || '320k'),
  fetchLxAudio: (url) => ipcRenderer.invoke('lx-source:fetch-audio', url),
});
